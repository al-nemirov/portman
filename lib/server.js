...
function snapshot() {
  ...
  const stats = proc.getStats(listening.map(e => e.pid));
  const cpuMap = activity.sampleCpu(stats);
  ...
  const items = listening.map(e => {
    ...
    const item = {
      ...
      cpuPct: cpuMap.has(e.pid) ? Math.round(cpuMap.get(e.pid) * 10) / 10 : null,
    };
