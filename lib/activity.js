...
const HIST_LEN = 30; // ~90s истории при 3s интервале
...
function snapshot(listening, stats) {
  ...
  for (const e of listening) {
    ...
    if (!entry.ramHist) entry.ramHist = [];
    if (!entry.cpuHist) entry.cpuHist = [];
    ...
    if (s) {
      if (entry.lastCpuTime != null && entry.lastCpuAt != null) {
        ...
        if (dtWall > 0.5) {
          ...
          entry.cpuHist.push(Math.round(pct * 10) / 10);
          if (entry.cpuHist.length > HIST_LEN) entry.cpuHist.shift();
        }
      }
      entry.lastCpuTime = s.cpuTime;
      entry.lastCpuAt = now;
      entry.ramHist.push(Math.round(s.ram / (1024 * 1024)));
      if (entry.ramHist.length > HIST_LEN) entry.ramHist.shift();
    }
    data[key] = entry;
  }
